...
describe('<Select />', () => {
  it('should render a h5 element if the headerText is not missing', () => {
    const props = {
  ...
  it('should not render a h5 element if the headerText is missing', () => {
    const props = {
      onSelectChange: () => {},
      options: [{ displayName: 'This is a test', value: 'test' }],
    };
    const wrapper = shallowWithIntl(<Select {...props} />);
    expect(wrapper.find('h5').length).to.equal(0);
  });
});
